...
const debug = /pkg-verify/.test(process.env.NODE_DEBUG)
  ? msg => console.error(`pkg-verify: ${msg}`)
  : () => {};
...
class Verifier {
  ...
  verify(name, dirname) {
    ...
    this.verifyPackage(pkg, dirname);
  }
...
function verify(dirname, name) {
  const v = new Verifier({ debug });
